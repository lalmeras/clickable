...
def _find_callable_name(mapping, name):
    if mapping is None:
        return 'main'
...
def _find_callable(module):
    """Extract callable from module. Print error messages and abort
    if some errors are encountered. Returns false if find failed."""
